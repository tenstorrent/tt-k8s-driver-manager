...
// Same migration contract as driver/v1alpha1: nodeAffinity wins,
// deprecated nodeSelector still works for CRs written pre-rename.
func TestEffectiveNodeAffinity_Firmware(t *testing.T) {
	cases := []struct {
		name        string
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			s := &TenstorrentFirmwarePolicySpec{NodeAffinity: tc.affinity, NodeSelector: tc.selector}
