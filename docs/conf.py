...
def _sync_chart_values(app, config):
    ...
    if src.is_file():
        text = src.read_text()
        # Render only the Requirements + Values tables: drop the generated header
        # (title, badges, homepage / maintainers / source-code links) and the
        # trailing autogenerated footer.
        m = re.search(r"^## Requirements", text, flags=re.M)
        if m:
            text = text[m.start():]
        text = re.sub(r"\n-{3,}\nAutogenerated from chart metadata using .*\Z", "\n", text, flags=re.S)
